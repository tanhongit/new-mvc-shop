                        ...
                        <div class="card">
                            <div class="body">
                                <ul class="comment-reply list-unstyled">
                                    <?php foreach ($orderDetail as $product) : ?>
                                        <li>
                                            ...
                                            <div class="text-box">
                                                ...
                                                <a style="padding-left: 20px;" href="product/<?= $product['product_id']; ?>-<?= $product['slug']; ?>">Giá sản phẩm: <?= number_format($product['product_price'], 0, ',', '.') ?>đ</a>
                                    ...
                                    <span style="font-size: 1.2em; float: right; padding-left: 25px; padding-right: 25px;"><b><a style="color: #fff" title="Tổng tiền" class="btn btn-primary btn-round"><i class="zmdi zmdi-money"></i> Tổng tiền: <?= number_format($order['cart_total'], 0, ',', '.') ?>đ</a></b></span>
                                    <span>
                                    ...
                                    <?php if ($order['status'] == 3 || $order['status'] == 1 || $order['status'] == 2) { ?>
                                        <span> <a href="admin.php?controller=feedback&action=add&order_id=<?= $order['id'] ?>" style="text-decoration: none;" class='btn btn-success waves-effect'>Phản hồi đơn hàng này</a></span>
                                    <?php } elseif ($order['status'] == 0) { ?>
                                        <span>
                                            <form enctype="multipart/form-data" action="admin.php?controller=purchase&action=cancell-action&order_id=<?= $order['id'] ?>" method="post">
                                                <button onclick="return confirm('Bạn có chắc chắn muốn hủy đơn hàng này?')" type="submit" class='btn btn-danger waves-effect'>Hủy đơn hàng này</button>
                                            </form>
                                        </span>
                                    <?php } ?>
                                </ul>
